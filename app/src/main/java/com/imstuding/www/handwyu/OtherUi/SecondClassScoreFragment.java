...
public class SecondClassScoreFragment extends Fragment {
    ...
    public void initFragment(View view){
        layout_second_score= view.findViewById(R.id.layout_second_score);
        keepword= view.findViewById(R.id.second_keeppwd);
        keepword.setChecked(true);
        id= view.findViewById(R.id.second_id);
        pwd= view.findViewById(R.id.second_pwd);
        initData();
        ...
        btn_second_score= view.findViewById(R.id.btn_second_score);
        ...
        public void run() {
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == 200) {
                    ...
                    if (response.contains("您的密码强度较弱") || !response.contains("top.leftFrame.location.href=\"menu.asp\";\n" +
                            "top.mainFrame.location.href=\"welcome.asp\";")){
                        myTableThread2 tableThread2=new myTableThread2();
                        tableThread2.start();
                    }else if(response.contains("登录信息不正确")){
                    ...
                    }else if (response.contains("短时间内连续登录失败")){
                        Message message=new Message();
                        Bundle bundle=new Bundle();
                        ...
                        Message message=new Message();
                        Bundle bundle=new Bundle();
                        bundle.putString("response","未知错误，退出app重登试试，或者等几小时再试试吧！"+response);
                        message.setData(bundle);
                        message.what=1014;
                        handle.sendMessage(message);
